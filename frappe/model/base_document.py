...
class BaseDocument(object):
    ...
    def get_valid_dict(self, sanitize=True, convert_dates_to_str=False, ignore_nulls=False):
        ...
        for fieldname in self.meta.get_valid_columns():
            ...
            if df:
                ...
                elif df.fieldtype in ("Currency", "Float", "Percent") and not isinstance(
                    d[fieldname], float
                ):
    ...
    def update_modified(self):
        ...
        frappe.db.set_value(
            self.doctype, self.name, "modified", self.modified, update_modified=False
        )
    ...
    def _get_missing_mandatory_fields(self):
        ...
        for df in self.meta.get("fields", {"reqd": ("=", 1)}):
            if (
                self.get(df.fieldname) in (None, [])
                or not strip_html(cstr(self.get(df.fieldname))).strip()
            ):
        ...
        for df in self.meta.get_link_fields() + self.meta.get(
            ...
            if docname:
                if df.fieldtype == "Link":
                    ...
                    if not doctype:
                        frappe.throw(
                            _("{0} must be set first").format(self.meta.get_label(df.options))
                        )
                ...
                if not frappe.get_meta(doctype).get("is_virtual"):
                    if not fields_to_fetch:
                        ...
                        values = frappe.db.get_value(
                            doctype, docname, values_to_fetch, as_dict=True
                        )
    ...
    def _validate_code_fields(self):
        for field in self.meta.get_code_fields():
            ...
            if language == "Python":
                frappe.utils.validate_python_code(
                    code_string, fieldname=field.label, is_expression=False
                )
    ...
    def _validate_update_after_submit(self):
        ...
        for key in self.as_dict():
            ...
            if df and not df.allow_on_submit and (self.get(key) or db_value):
                ...
                if type(self_value) == datetime.timedelta and type(db_value) == datetime.time:
                    self_value = (datetime.datetime.min + self_value).time()
    ...
    def _sanitize_content(self):
        ...
        for fieldname, value in self.get_valid_dict().items():
            ...
            elif "<!-- markdown -->" in value and not bool(
            ...
            if df and (
                ...
                sanitized_value = sanitize_html(
                    value, linkify=df and df.fieldtype == "Text Editor"
                )
    ...
    def _save_passwords(self):
        ...
        for df in self.meta.get("fields", {"fieldtype": ("=", "Password")}):
            if (
                ...
                and df.fieldname in self.flags.ignore_save_passwords
            ):
                continue
    ...
    def is_print_hide(self, fieldname, df=None, for_print=True):
        ...
        if self.get(fieldname) == 0 and not self.meta.istable:
            print_hide = (df and df.print_hide_if_no_value) or (
                meta_df and meta_df.print_hide_if_no_value
            )
