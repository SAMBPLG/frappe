...
def migrate(context, skip_failing=False, skip_search_index=False):
    ...
    for site in context.sites:
        ...
        try:
            migrate(
                context.verbose, skip_failing=skip_failing, skip_search_index=skip_search_index
            )
        finally:
            print()
            frappe.destroy()
...
def backup(
    ...
    for site in context.sites:
        try:
            ...
            click.secho(
                "Backup failed for Site {0}. Database or site_config.json may be corrupted".format(
                    site
                ),
                fg="red",
            )
...
def _drop_site(
    ...
    try:
        ...
        if force:
            ...
            messages = [
                "=" * 80,
                "Error: The operation has stopped because backup of {0}'s database failed.".format(
                    site
                ),
                "Reason: {0}\n".format(str(err)),
